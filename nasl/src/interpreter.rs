...
trait CloneableFn: Fn(Vec<FunctionArgument>, Option<RuntimeParameter>) -> Result<ResultType, Box<dyn Error>>{
...
where
    F: Fn(Vec<FunctionArgument>, Option<RuntimeParameter>) -> Result<ResultType, Box<dyn Error>> + Clone,
{
    fn clone_box<'a>(&self) -> Box<dyn 'a + CloneableFn>
    where
        Self: 'a,
...
impl NASLFunctions {
    ...
    pub fn run(self, name: &str, args: Vec<FunctionArgument>) -> Result<ResultType, Box<dyn Error>> {
        let fuck_borrowing = &Func {
            name: name.to_string(),
            arg_len: args.len(),
            runner: Box::new(| _a, _p | Err(Box::new( UnexpectedError { description: "bla".to_string() } )))
        };
        let func = self.functions.iter().find(| x | x.name == name.to_string() ).unwrap_or(fuck_borrowing);
...
impl UnexpectedError {
    pub fn new(name: &str, args_len: usize, expected: usize) -> Self {
        UnexpectedError { description: format!("{} expects {} arguments but got {}", name, args_len, expected) }
    }
}
            ...
            Token::Word(a) => {
                match state {
                    ...
                    State::InFunction(f, ar) => {
                        let mut args = ar;
                        args.push(FunctionArgument { name: None, value: a });
